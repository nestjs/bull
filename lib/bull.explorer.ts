...
export class BullExplorer {
                ...
                .scanFromPrototype(
                    ...
                    (key: string) => {
                        if (BullExplorer.isMethodDecoratedAsProcessor(instance, key)) {
                            const options = BullExplorer.getProcessorMetadata(instance, key) || {};
                        ...
                        else if (BullExplorer.isMethodDecoratedAsEventListener(instance, key)) {
                            const options = BullExplorer.getEventListenerMetadata(instance, key) || {};
    ...
    private static isMethodDecoratedAsEventListener(instance: Injectable, methodKey: string): boolean {
        return Reflect.hasMetadata(BULL_MODULE_ON_EVENT, instance, methodKey);
    }

    private static getProcessorMetadata(instance: Injectable, methodKey: string): any {
        return Reflect.getMetadata(BULL_MODULE_PROCESS, instance, methodKey);
    }

    private static getEventListenerMetadata(instance: Injectable, methodKey: string): any {
