...
import {BullModuleOptions, BullModuleAsyncOptions, BullOptionsFactory} from './bull.interfaces';
...
export function createAsyncQueuesProviders(options: BullModuleAsyncOptions[]): Provider[] {
  ...
  return options.map((option: BullModuleAsyncOptions): Provider => {
    if (option.useFactory) {
      return {
        provide: getQueueToken(option.name),
        useFactory: async (...args: any[]) => buildQueue(await option.useFactory(...args)),
    ...
    return {
      provide: getQueueToken(option.name),
      useFactory: async (optionFactory: BullOptionsFactory) => buildQueue(await optionFactory.createBullOptions()),
